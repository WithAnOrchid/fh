...
const scan_param = "sudo arp-scan --interface=wlan0 --localnet | sort -V | grep -P '^[0-9]\{2,3\}' | awk -v OFS=\",\" '{ print $1, $2 }'"
            ...
            if (connection === true) {
                ...
                function test() {
                    ...
                    var p = new Promise((resolve, reject) => {
                        exec(scan_param, function (err, report) {
                            if (err) {
                                console.log("error");
                                reject(err)
                            } else {
                    ...
                    p.then((results) => {
                        console.log(results);
                    logger.info('SCAN RESULT : ' + results);

                }).catch((err) => {
                        console.log("-*-*- ERROR scan -*-*-");
                    logger.info('SCAN FAILED : ' + err);
                    console.log(err);
                })
